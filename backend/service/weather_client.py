...
def get_current_weather(nx, ny):
    ...
    print(
        "🔍 현재 날씨 요청: base_date={base_date}, base_time={base_time}, "
        "nx={nx}, ny={ny} (원래시각: {now_time})".format(
            base_date=base_date,
            base_time=base_time,
            nx=nx,
            ny=ny,
    ...
    for item in items:
        if item['category'] == 'T1H':
            data["기온(℃)"] = item['obsrValue']
        if item['category'] == 'REH':
            data["습도(%)"] = item['obsrValue']
        if item['category'] == 'WSD':
            data["풍속(m/s)"] = item['obsrValue']
        if item['category'] == 'RN1':
            data["강수량(mm)"] = item['obsrValue']
    return data
...
def get_short_forecast(nx, ny):
    ...
    print(
        "🔍 단기예보 요청: base_date={base_date}, base_time={base_time}, nx={nx}, "
        "ny={ny} (현재시각: {current_hour}시)".format(
            base_date=base_date,
            base_time=base_time,
            nx=nx,
            ny=ny,
